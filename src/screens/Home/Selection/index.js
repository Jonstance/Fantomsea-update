...
const Selection = ({featuredNFTs}) => {

  const [featuredNftItems, setFeaturedNftItems] = useState([])

  function generateItems(featurnedNfts){

    const returendDesiredData = featurnedNfts.map(eachNftObject=>{
      return {
    ...
    image2x: eachNftObject.nftDigitalUrl
      }
    })

    return returendDesiredData
  ...
  useEffect(()=>{
    ...
    setFeaturedNftItems(generateItems(featuredNFTs))
